...
- "MEDICAID FOR DME ITEMS" → "MEDICAID"
- "EPACES" → "MEDICAID"
"""
import sqlite3
import shutil
...
def preview_changes():
    ...
    cur.execute("""
        ...
        FROM patients 
        WHERE primary_insurance IN ('MEDICAID FOR DME ITEMS', 'EPACES')
    ...
    if primary_records:
        ...
        for rec in primary_records:
            print(f"  ID {rec[0]:4} | {rec[2]}, {rec[1]:20} | {rec[3]:30} → MEDICAID | Policy: {rec[4] or 'None'}")
    
    # Check secondary insurance
    cur.execute("""
        SELECT id, first_name, last_name, secondary_insurance, secondary_insurance_id
        FROM patients 
        WHERE secondary_insurance IN ('MEDICAID FOR DME ITEMS', 'EPACES')
        ORDER BY last_name, first_name
    """)
...
def apply_updates():
    ...
    cur.execute("""
        UPDATE patients 
        SET primary_insurance = 'MEDICAID'
        WHERE primary_insurance IN ('MEDICAID FOR DME ITEMS', 'EPACES')
    """)
    ...
    cur.execute("""
        UPDATE patients 
        SET secondary_insurance = 'MEDICAID'
        WHERE secondary_insurance IN ('MEDICAID FOR DME ITEMS', 'EPACES')
    """)
...
def verify_updates():
    ...
    cur.execute("""
        ...
        WHERE primary_insurance IN ('MEDICAID FOR DME ITEMS', 'EPACES')
           OR secondary_insurance IN ('MEDICAID FOR DME ITEMS', 'EPACES')
    """)
    ...
    # Count new MEDICAID entries
    cur.execute("SELECT COUNT(*) FROM patients WHERE primary_insurance = 'MEDICAID'")
    primary_medicaid = cur.fetchone()[0]
    
    cur.execute("SELECT COUNT(*) FROM patients WHERE secondary_insurance = 'MEDICAID'")
    secondary_medicaid = cur.fetchone()[0]
    
    print(f"📊 Current MEDICAID counts:")
    print(f"   Primary: {primary_medicaid} patients")
    print(f"   Secondary: {secondary_medicaid} patients")
    
    conn.close()

if __name__ == "__main__":
    print("\n" + "="*70)
    print("INSURANCE NAME UPDATE UTILITY")
    print("="*70)
